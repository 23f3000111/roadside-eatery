...
export default function Hero() {
  ...
  return (
    <section
      id="home"
      ref={ref}
      className="relative min-h-screen flex items-center overflow-hidden bg-[#FBF6EF]"
    >
      {/* Background warm gradient blobs */}
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 w-[65%] h-full bg-gradient-to-l from-[#fdece2] via-[#fef6f1] to-transparent" />
        <div className="absolute bottom-0 left-0 w-80 h-80 rounded-full bg-[#E8622A] opacity-[0.06] blur-3xl float-blob" />
        <div className="absolute top-24 right-1/3 w-64 h-64 rounded-full bg-[#E8A33D] opacity-[0.08] blur-3xl float-blob" style={{ animationDelay: '4s' }} />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-5 md:px-8 w-full pt-32 md:pt-28 pb-16 md:pb-12 grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-12 items-center min-h-screen">
        ...
        >
          ...
          <motion.div
            ...
            animate="show"
            custom={0.2}
            className="inline-flex items-center gap-2 bg-white border border-[#E8622A]/25 rounded-full px-4 py-1.5 mb-7 shadow-sm"
          >
            <MapPin size={13} className="text-[#E8622A]" />
            <span className="font-manrope text-xs font-semibold text-[#C24A18] tracking-widest uppercase">
          ...
          <motion.h1
            ...
            animate="show"
            custom={0.35}
            className="font-playfair text-[2.9rem] leading-[1.04] sm:text-6xl xl:text-7xl font-bold text-[#1F1B18] mb-6"
          >
            Grilled Comfort,
          ...
          <motion.p
            ...
            animate="show"
            custom={0.5}
            className="font-manrope text-base md:text-lg text-[#6B5F55] leading-relaxed max-w-md mb-9"
          >
          ...
          >
            <div className="flex items-center gap-1.5">
              {[...Array(5)].map((_, i) => (
                <Star key={i} size={14} fill="#E8A33D" className="text-[#E8A33D]" />
              ))}
              <span className="font-manrope font-bold text-sm text-[#1F1B18] ml-1">4.8</span>
            </div>
            <span className="text-[#E8A33D]">✦</span>
            <span className="font-manrope text-sm text-[#6B5F55]">300+ Reviews</span>
            <span className="text-[#E8A33D]">✦</span>
            <span className="font-manrope text-sm text-[#6B5F55]">Family &amp; Friends Fav</span>
          </motion.div>
        </motion.div>
        ...
        >
          ...
          <div className="absolute -z-0 -bottom-7 -right-6 w-36 h-36 rounded-full border-[20px] border-[#E8622A]/12 pointer-events-none" />
          ...
          <motion.div style={{ y: mosaicY }} className="relative z-[1] flex flex-col gap-3">
            ...
            >
              ...
              <div className="absolute bottom-5 left-6 right-6">
                <span className="font-manrope text-[10px] tracking-[0.25em] uppercase font-semibold text-[#FFB88C]">
                  ✦ Signature Plate
                </span>
          ...
          {/* Single floating accent: rating coin (bottom-left, clear of navbar) */}
          <motion.div
            initial={{ opacity: 0, scale: 0.7, y: 16 }}
            animate={{ opacity: 1, scale: 1, y: 0 }}
            transition={{ duration: 0.6, delay: 1.15, type: 'spring', stiffness: 180 }}
            className="absolute z-[2] -bottom-5 -left-3 md:-left-6 bg-white rounded-2xl px-4 py-3 shadow-[0_16px_40px_rgba(31,27,24,0.18)] flex items-center gap-3"
          >
            <span className="text-2xl md:text-3xl font-playfair font-bold text-[#1F1B18]">4.8</span>
            <div className="flex flex-col">
              <div className="flex">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={11} fill="#E8A33D" className="text-[#E8A33D]" />
                ))}
              </div>
              <span className="font-manrope text-[10px] text-[#6B5F55]">Google · 300+ reviews</span>
            </div>
          </motion.div>
        </motion.div>
      </div>
      ...
      >
        <span className="font-manrope text-xs text-[#6B5F55] tracking-widest uppercase">Scroll</span>
        <motion.div
          animate={{ y: [0, 8, 0] }}
          transition={{ repeat: Infinity, duration: 1.6, ease: 'easeInOut' }}
          className="w-0.5 h-8 bg-gradient-to-b from-[#E8622A] to-transparent rounded-full"
        />
      </motion.div>
    </section>
  );
